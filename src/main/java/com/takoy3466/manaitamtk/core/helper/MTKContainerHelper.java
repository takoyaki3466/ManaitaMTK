package com.takoy3466.manaitamtk.core.helper;
...
public class MTKContainerHelper {
    ...
    public static <T extends ItemStackHandler> ItemStack removeItem(T stacks, int slotId, int count) {
        return slotId >= 0 && slotId < stacks.getSlots() && !stacks.getStackInSlot(slotId).isEmpty() && count > 0 ? stacks.getStackInSlot(slotId).split(count) : ItemStack.EMPTY;
    }

    public static ItemStack takeItem(MTKItemStackHandler stacks, int slotId) {
        return slotId >= 0 && slotId < stacks.getSlots() ? stacks.set(slotId, ItemStack.EMPTY) : ItemStack.EMPTY;
    }
}
